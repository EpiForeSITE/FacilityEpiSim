...
The model follows a standard **agent-based modeling architecture** using Repast Simphony:
...
- `updateDailyStatistics()` - Called each day to update outputs
- `doAdmit()` - Scheduled admission process
- `saveDischargedPatients()` - Records discharged patients

**Responsibilities**:
- Creates Facility agent
- Manages simulation schedule
- Writes output files (when not in batch mode)
- Handles burn-in vs. measurement period logic
...
```
1. RunEnvironment fires scheduled methods
2. Facility.updatePopulationTally() records daily stats
3. SingleFacilityBuilder.updateDailyStatistics() writes outputs
4. Any scheduled events fire (admissions, discharges, detections)
5. Transmission rates updated if needed
6. Clock advances to next tick
```
...
**Event logs** (admissions.txt, transmissions.txt, etc.)
```
