...
4. **Wait** for "Simulation completed" message (typically 5-15 minutes)

### Command Line Method

```bash
make compile  # If not already done
make run-batch
```
...
Summary statistics from the entire run:

```
Total Admissions: 12,500
Total Patient-Days: 3,250,000
Mean LOS: 26.0 days
Average Daily Census: 297.8

Measurement Period Results:
  Admissions (5-year): 4,250
  Patient-Days (5-year): 1,085,000

Disease Prevalence (5-year average): 12.3%
Disease Incidence: 8.7 per 100 admissions

Detected Cases: 567
Surveillance Detections: 423
Clinical Detections: 144
```
...
1. ✅ Population stabilizes around target (after burn-in)
2. ✅ Prevalence oscillates within expected range (no trends)
3. ✅ Detection ratio matches surveillance adherence
4. ✅ Event logs show continuous admission/discharge pattern

**Warning signs:**

1. ⚠️ Population declining → Admission rate too low
2. ⚠️ Prevalence trending upward → Increasing transmission
3. ⚠️ Few detection events → Check surveillance settings
4. ⚠️ Few transmission events → Beta too low
...
daily_stats <- read_csv("daily_population_stats.txt")
...
```r
# Load admissions
admissions <- read.csv("admissions.txt", sep = ",")

# Count imported vs. acquired cases
admitted <- read.table("admissions.txt", sep = ",", header = FALSE)
